...
STATE = {"query": "", "list": [], "page": 0, "popup": None}
...
def render(screen, fonts):
    ...
    title = f"{TITLE_PREFIX}:{STATE['query']}"
...
def handle(data, screen, fonts):
    ...
    for i in range(PER_PAGE):
        ...
        if _hit((cx, cy, cw, cell_h), x, y):
            item = STATE["list"][idx]
            print(f"[结果] 点击 {item.get('name')} → 详情页")
            from .result_page import info
            info.STATE.update(item=item, info=None, error=None, page=0, popup=None,
                                group=0, from_page="result", is_fav=False)
            STATE["popup"] = ["加载中…"]
            _show(screen, fonts)
            def _run():
                try:
