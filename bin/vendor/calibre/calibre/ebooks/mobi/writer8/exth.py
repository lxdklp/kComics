...
def build_exth(
    ...
    # Write UUID as ASIN. 即使作为个人文档（share_not_sync=True）也写入，
    # 以便 Kindle 生成锁屏封面缩略图 thumbnail_<uuid>_<cdetype>_portrait.jpg。
    ...
    if not is_periodical:
        # 个人文档用 PDOC（书架封面正常显示），普通书用 EBOK。
        cdetype = b'EBOK' if not share_not_sync else b'PDOC'
